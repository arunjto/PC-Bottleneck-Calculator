document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!dom.calculatorView) {
        // We are on a different page (e.g. about.html), so we only need the theme switcher logic.
        setupTheme();
        return;
    }

    // --- HELPER FUNCTION ---
    // This function intelligently gets the score and image URL.
    // It can handle both formats: "Name": 100 AND "Name": { score: 100, imageUrl: "..." }
    function getComponentInfo(type, name) {
        const componentData = data[type][name];
        if (typeof componentData === 'object' && componentData !== null) {
            // New format: Return score and specific image
            return { score: componentData.score, imageUrl: componentData.imageUrl };
        }
        // Old format: Return score and the generic placeholder image
        return { score: componentData || 0, imageUrl: placeholderImages[type.slice(0, 3)] };
    }

    function populateDropdown(selectElement, componentData) {
        const sortedKeys = Object.keys(componentData).sort((a, b) => a.localeCompare(b));
    ...
    function calculateBottleneck() {
        ...
        // Use the helper function to get component details
    ...
    // --- RENDER FUNCTION ---
    // Updated to use the dynamic image URLs passed from calculateBottleneck
        ...
        dom.resultsView.innerHTML = `
            ...
            <div class="component-analysis">
                <div class="component-card">
                    <h4>Processor (CPU)</h4>
                    <img src="${cpuInfo.imageUrl}" alt="${selected.cpu}">
                ...
                <div class="component-card">
                    ...
                    <img src="${gpuInfo.imageUrl}" alt="${selected.gpu}">
    ...
    function setupTheme() {
        if (!dom.themeToggle) return; // Failsafe for other pages
