...
const COPY: Record<Locale, ToolsPageCopy> = {
  ...
  ru: {
    hubTitle: "Бесплатные калькуляторы для ПК и инструменты для обновления",
    hubDescription: "Планируйте обновление GPU, CPU, памяти, дисплея, настроек и хранилища с помощью прозрачных калькуляторов.",
    hubIntro: "Выберите нужный инструмент ниже. Каждый результат показывает основу расчета, предположения и ограничения, поэтому вы можете проверить их перед покупкой оборудования.",
    home: 'Главная', tools: 'Инструменты', openTool: "Открыть калькулятор", formula: "Используемая формула или модель", methodology: "Методика расчёта",
    methodologyIntro: "Калькулятор запускается в вашем браузере на основе выбранных вами значений. В оценках используются прозрачные правила планирования и нормализованные данные об оборудовании объекта; они не являются лабораторными эталонами.",
    calculationSteps: "Этапы расчета", limitations: "Предположения и ограничения", resultMeaning: "Как интерпретировать результат",
    relatedTools: "Сопутствующие инструменты для ПК", relatedDescription: "Продолжайте с помощью калькулятора, который отвечает на следующую часть того же решения об обновлении.",
    viewAllTools: "Просмотреть все инструменты для ПК", reviewed: "Проверка методологии: 17 июля 2026 г.", coreHeading: "Базовые калькуляторы PCBuildCheck",
    categories: {
      upgrade: { title: "Планирование обновления", description: "Сравните компоненты и определите, какое обновление заслуживает внимания в первую очередь." },
      performance: { title: "Игровая производительность и дисплей", description: "Поймите FPS, время кадра, частоту обновления, масштабирование разрешения и настройки." },
      memory: { title: "Планирование памяти", description: "Оцените объем графической и системной памяти для вашей рабочей нагрузки." },
      storage: { title: "Планирование хранения", description: "Сравните типы хранилищ и спланируйте полезную емкость игровой библиотеки." },
    },
    coreTools: {
      bottleneck: { title: "Калькулятор узких мест ПК", description: "Сравните баланс CPU и GPU с основной моделью PCBuildCheck." },
      fps: { title: "Калькулятор FPS в играх", description: "Оцените общий диапазон FPS для выбранного оборудования и игры." },
      psu: { title: "Калькулятор мощности блока питания", description: "Оцените энергопотребление системы и практический запас мощности блока питания." },
    },
  },
};
