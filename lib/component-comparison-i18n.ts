...
export const COMPONENT_COMPARISON_COPY: Record<Locale, ComponentComparisonCopy> = {
  ...
  ru: {
    configureTitle: "Выберите два компонента",
    configureDescription: "Выберите класс компонентов и сравните две модели одного класса. Первая модель является базовой.",
    cpuTab: "Сравнить процессоры", gpuTab: "Сравнить графические процессоры", firstCpu: "Базовый уровень CPU", secondCpu: "Сравнение CPU",
    firstGpu: "Базовый уровень GPU", secondGpu: "Сравнение GPU", selectCpu: "Выберите CPU.", selectGpu: "Выберите GPU.",
    searchCpu: "Поиск процессоров по названию, уровню или спецификации", searchGpu: "Поиск графических процессоров по названию, уровню или спецификации",
    noCpuResults: "Никакой CPU не соответствует {query}", noGpuResults: "Никакой GPU не соответствует {query}",
    openCpuInstructions: "Откройте список CPU. Используйте поиск или клавиши со стрелками, чтобы выбрать процессор.",
    openGpuInstructions: "Откройте список GPU. Используйте поиск или клавиши со стрелками, чтобы выбрать видеокарту.",
    cpuListLabel: "Доступные процессоры", gpuListLabel: "Доступные видеокарты",
    swap: "Поменять компоненты местами", copyLink: "Скопировать ссылку для сравнения", copied: "Ссылка скопирована",
    copyError: "Ссылку не удалось скопировать. Вместо этого скопируйте URL-адрес из адресной строки.",
    resultsTitle: "Параллельное сравнение", baseline: 'Базовый компонент', comparison: 'Сравниваемый компонент',
    normalizedScore: "Нормализованный показатель планирования", coresThreads: "Ядра/потоки", baseBoostClock: "Базовая/ускоренная частота",
    listedPower: "Заявленная мощность", socket: 'Сокет', architecture: 'Архитектура', releaseYear: "Год выпуска", tier: 'Класс',
    vram: "Графическая память", officialSpecs: "Официальные характеристики", relativeChanges: "Относительные изменения по сравнению с исходным уровнем",
    scoreDifference: "Разница очков", powerDifference: "Разница в мощности", efficiencyDifference: "Индекс оценки на ватт",
    coreDifference: "Основное отличие", threadDifference: "Разница в резьбе", vramDifference: "VRAM разница",
    categoryLeaders: "Лидеры категорий", metric: 'Показатель', leader: 'Лидер', higherScore: "Более высокий нормализованный балл",
    powerEfficiency: "Более высокий индекс оценки на ватт", lowerPower: "Более низкая заявленная мощность", moreCores: "Больше ядер CPU",
    moreThreads: "Больше потоков CPU", moreVram: "Больше графической памяти", tie: 'Ничья',
    scoreHigher: "{component} имеет нормализованную оценку планирования на {value}% выше, чем {baseline}.",
    scoreLower: "{component} имеет нормализованную оценку планирования на {value}% ниже, чем {baseline}.",
    scoreSame: "Оба компонента имеют одинаковую нормализованную оценку планирования.",
    socketSame: "Оба процессора имеют сокет {socket}. Перед покупкой подтвердите точный набор микросхем материнской платы и поддержку BIOS.",
    socketDifferent: "Перечисленные сокеты CPU различаются. Может потребоваться смена материнской платы или платформы.",
    socketUnknown: "Информация о сокете неполная. Проверьте оба процессора на соответствие списку поддерживаемых материнских плат.",
    modelNotice: "Оценки — это индексы редакционного планирования, а не измеренные контрольные проценты. Не сравнивайте результаты CPU напрямую с результатами GPU.",
    dataSnapshot: "Снимок данных оборудования", methodologyVersion: "Версия методологии оценки",
    emptyState: "Выберите два разных компонента, чтобы просмотреть сравнение.",
  },
};
