...
const UI_COPY: Record<Locale, UiCopy> = {
  ...
  ru: buildCopy('ru', {
    calculator: "Введите вашу конфигурацию", results: "Расчётный результат", calculate: 'Рассчитать', reset: 'Сбросить',
    estimateNotice: "Только оценка планирования — проверяйте важные решения по обновлению с помощью измерений, проверок совместимости и независимых тестов.",
  }, {
    currentGpu: 'Текущая видеокарта', newGpu: 'Новая видеокарта', cpu: 'Текущий процессор', currentFps: 'Текущий FPS или исходная производительность',
    currentCpu: 'Текущий процессор', newCpu: 'Новый процессор', gpu: 'Текущая видеокарта', useCase: 'Основной сценарий использования',
    gameType: 'Игровая нагрузка', resolution: 'Разрешение', textureQuality: 'Качество текстур', rayTracing: 'Трассировка лучей',
    monitorCount: 'Активные мониторы', upscaling: 'Режим масштабирования', multitasking: 'Одновременная многозадачность', streaming: 'Стриминг во время игры',
    modding: 'Уровень модификаций', targetFps: 'Целевой FPS', refreshRate: 'Частота обновления монитора',
    baseResolution: 'Базовое разрешение дисплея', targetResolution: 'Разрешение для сравнения', scalePercent: 'Масштаб рендеринга',
    ramCapacity: 'Установленная RAM', game: 'Игра', currentStorage: 'Текущий накопитель', newStorage: 'Новый накопитель',
    driveCapacity: 'Ёмкость нового накопителя', systemSpace: 'Резерв для системы и приложений', librarySize: 'Текущая библиотека игр',
    averageGameSize: 'Средний размер игры', storageType: 'Тип текущего накопителя',
  }, {
    expectedFps: 'Расчётный эффективный FPS', usefulGain: 'Расчётный полезный прирост', theoreticalGain: 'Разница исходных показателей',
    vramChange: 'Изменение VRAM', powerChange: 'Изменение энергопотребления видеокарты', cpuLimit: 'Проверка запаса CPU',
    expectedOutcome: 'Расчётное значение производительности', coreChange: 'Изменение числа ядер', gpuLimit: 'Проверка запаса GPU',
    recommendedVram: 'Рекомендуемый типичный объём VRAM', planningRange: 'Расчётный рабочий диапазон', workingVram: 'Расчётная середина диапазона', headroom: 'Запас при рекомендуемом объёме',
    gameProfileBase: 'База игрового профиля', resolutionAllowance: 'Поправка на разрешение', textureAllowance: 'Поправка на текстуры',
    rayTracingAllowance: 'Поправка на трассировку лучей', upscalingAdjustment: 'Поправка на масштабирование',
    recommendedRam: 'Рекомендуемый объём RAM', workingRam: 'Расчётное использование RAM', currentFrameTime: 'Текущее время кадра',
    targetFrameTime: 'Целевое время кадра', frameTimeDifference: 'Разница с целевым значением',
    displayUtilization: 'Использование частоты обновления', fpsSurplus: 'FPS сверх частоты обновления', refreshHeadroom: 'Неиспользованный запас частоты обновления',
    renderInterval: 'Интервал рендеринга', refreshInterval: 'Интервал обновления дисплея', renderedResolution: 'Внутреннее разрешение рендеринга',
    renderedPixels: 'Отрисовываемые пиксели', nativeWorkloadChange: 'Изменение нагрузки относительно базового разрешения', targetWorkloadChange: 'Изменение нагрузки относительно сравниваемого разрешения',
    suggestedPreset: 'Рекомендуемый пресет качества', presetFps: 'Расчётный FPS с пресетом', ultraFps: 'Расчётный FPS на Ultra',
    limitingComponent: 'Вероятно более медленный компонент', ramStatus: 'Проверка объёма памяти', responsivenessGain: 'Относительный прирост отзывчивости',
    freeCapacity: 'Доступная ёмкость после резервов', additionalGames: 'Примерное число дополнительных игр', upgradeClass: 'Сводка обновления',
    firstPriority: '1-й приоритет', secondPriority: '2-й приоритет', thirdPriority: '3-й приоритет', fourthPriority: '4-й приоритет',
    currentPsu: 'Расчётная мощность БП текущей сборки', proposedPsu: 'Расчётная мощность БП новой сборки',
    socketChange: 'Указанный сокет CPU', platformCheck: 'Проверка платформы',
  }, {
    gaming: 'Игры', productivity: 'Работа и создание контента', everyday: 'Повседневное использование', esports: 'Киберспорт',
    streamingCreator: 'Игры и стриминг', creator: 'Создание контента', aaa: 'Современные AAA-игры', mainstream: 'Популярные игры',
    modded: 'Игры с тяжёлыми модами / симуляторы', medium: 'Среднее', high: 'Высокое', ultra: 'Ультра', none: 'Выкл. / нет',
    light: 'Лёгкая', heavy: 'Высокая', one: '1 монитор', two: '2 монитора', three: '3 монитора',
    native: 'Нативное разрешение / выкл.', quality: 'Режим «Качество»', balancedMode: 'Сбалансированный режим', performance: 'Режим «Производительность»',
    multitaskLight: 'Лёгкая: голосовой чат и несколько вкладок', multitaskMedium: 'Умеренная: много вкладок и фоновых приложений',
    multitaskHeavy: 'Высокая: несколько активных приложений', yes: 'Да', no: 'Нет',
    modLight: 'Несколько лёгких модов', modHeavy: 'Большая сборка модов / текстуры высокого разрешения',
    hdd: 'Жёсткий диск (HDD)', sata: 'SATA SSD', nvme3: 'NVMe SSD (PCIe 3.0)', nvme4: 'NVMe SSD (PCIe 4.0+)',
  }, {
    likely: "Вероятное ограничение", noLikely: "Выраженное ограничение не выявлено", cpu: 'CPU', gpu: 'GPU', balanced: 'Сбалансировано',
    sufficient: "Емкость соответствует базовой линии игры или превышает ее.", pressure: "Ниже базовой линии игры; пейджинг может снизить согласованность",
    competitive: 'Соревновательный', low: 'Низкое', medium: 'Среднее', high: 'Высокое', ultra: 'Ультра',
    hddToSsd: "Значительное повышение скорости отклика с жесткого диска на твердотельное хранилище.", ssdStep: "Более быстрый уровень хранения; прирост игровой нагрузки варьируется",
    sameTier: "Аналогичный класс хранения; емкость может быть основным преимуществом", noUrgent: "Отсутствие большого смоделированного дефицита",
    componentCpu: 'CPU', componentGpu: 'GPU', componentRam: 'RAM', componentStorage: 'Накопитель',
    unknown: 'Неизвестно', sameSocket: "Указан тот же разъём; проверьте точную модель материнской платы и список поддержки BIOS",
    platformChange: "Другая розетка; возможна замена материнской платы или платформы", verifyPlatform: "Данные сокета неполные; проверьте платформу вручную",
    checklistTitle: "Проверка совместимости перед покупкой",
    cpuSocketSame: "Оба процессора имеют список {socket}, но соответствующий сокет не гарантирует поддержку набора микросхем или BIOS.",
    cpuSocketChanged: "Указанный сокет изменится с {current} на {proposed}; вероятна смена материнской платы или платформы.",
    cpuSocketUnknown: "Информация о сокете неполная. Подтвердите оба процессора и точный список поддерживаемых материнских плат.",
    cpuBios: "Подтвердите модель материнской платы, набор микросхем, версию BIOS, поколение RAM и список поддержки CPU.",
    cpuCooling: "Проверьте крепление кулера, тепловую мощность, зазор корпуса и пределы мощности.",
    gpuPower: "Подтвердите мощность PSU и рекомендации по собственному разъему от производителя GPU и партнера по плате.",
    gpuClearance: "Проверьте длину и толщину карты, зазор в слоте, опорный кронштейн и циркуляцию воздуха в корпусе.",
    gpuDrivers: "Проверьте выходы дисплея, поддержку драйверов и наличие любого необходимого адаптера или родного кабеля.",
    openPsu: "Открыть предварительно заполненный калькулятор PSU",
  }),
};
...
const VRAM_UI_OVERRIDES: Record<Locale, VramUiOverrides> = {
  ...
  ru: {
    upscaling: "Режим масштабирования",
    results: {
      recommendedVram: "Предлагаемый общий уровень VRAM", planningRange: "Предполагаемый рабочий диапазон",
      workingVram: "Средняя точка планирования", headroom: "Запас на рекомендуемом уровне", gameProfileBase: "База игровых профилей",
      resolutionAllowance: "Допуск на разрешение", textureAllowance: "Припуск текстуры",
      rayTracingAllowance: "Допуск трассировки лучей", upscalingAdjustment: "Регулировка масштабирования",
    },
    options: { native: "Родное разрешение/выкл.", quality: "Режим качества", balancedMode: "Сбалансированный режим", performance: "Режим производительности" },
  },
};
