...
const COPY: Record<Locale, MethodologyCopy> = {
  en: {
    ...
    variants: {
      ...
      psu: {
        summary: 'The PSU calculator estimates system load from published CPU/GPU power and transparent allowances for the components actually selected.',
        inputs: 'Published CPU and GPU power, motherboard baseline, RAM, drives, fans, cooling, optional devices, a second GPU, and manual or overclocking allowances.',
        calculation: 'The selected component allowances are summed with CPU and GPU power. The tool shows 15%, 30% and 50% headroom scenarios and rounds the planning result to a common PSU wattage where possible.',
        formula: 'Planning wattage = next common PSU size ≥ (CPU + GPU(s) + selected component allowances) × 1.30',
  ...
  it: {
    ...
    variants: {
      ...
      psu: {
        summary: 'Stima il carico da CPU e GPU e da quote trasparenti per i componenti realmente selezionati.',
        inputs: 'Potenza di CPU e GPU, base della scheda madre, RAM, unità, ventole, raffreddamento, dispositivi opzionali, seconda GPU e margini manuali o di overclock.',
        calculation: 'Somma le quote selezionate alla potenza di CPU e GPU; mostra scenari con margine del 15%, 30% e 50% e arrotonda a un wattaggio PSU comune.',
        formula: 'Wattaggio pianificato = taglio PSU comune ≥ (CPU + GPU + quote selezionate) × 1,30',
  ...
  fr: {
    ...
    variants: {
      ...
      psu: {
        summary: 'Estime la charge depuis les puissances CPU/GPU et des réserves transparentes pour les composants réellement sélectionnés.',
        inputs: 'Puissances CPU et GPU, base carte mère, RAM, disques, ventilateurs, refroidissement, appareils facultatifs, second GPU et réserves manuelles ou d’overclocking.',
        calculation: 'Additionne les réserves sélectionnées aux puissances CPU et GPU; présente 15%, 30% et 50% de marge puis arrondit à une puissance PSU courante.',
        formula: 'Puissance planifiée = taille PSU courante ≥ (CPU + GPU + réserves sélectionnées) × 1,30',
  ...
  de: {
    ...
    variants: {
      ...
      psu: {
        summary: 'Schätzt die Systemlast aus CPU/GPU-Leistung und transparenten Pauschalen für die tatsächlich ausgewählten Komponenten.',
        inputs: 'CPU- und GPU-Leistung, Mainboardbasis, RAM, Laufwerke, Lüfter, Kühlung, optionale Geräte, zweite GPU sowie manuelle oder Übertaktungsreserven.',
        calculation: 'Addiert die gewählten Komponentenpauschalen zu CPU und GPU; zeigt 15%, 30% und 50% Spielraum und rundet auf eine übliche Netzteilgröße.',
        formula: 'Planungsleistung = übliche PSU-Größe ≥ (CPU + GPU(s) + gewählte Komponenten) × 1,30',
  ...
  es: {
    ...
    variants: {
      ...
      psu: {
        summary: 'Estima la carga desde la potencia de CPU/GPU y asignaciones transparentes para los componentes realmente seleccionados.',
        inputs: 'Potencia de CPU y GPU, base de placa, RAM, unidades, ventiladores, refrigeración, dispositivos opcionales, segunda GPU y márgenes manuales o de overclock.',
        calculation: 'Suma las asignaciones seleccionadas a CPU y GPU; muestra márgenes del 15%, 30% y 50% y redondea a una potencia PSU habitual.',
        formula: 'Potencia planificada = tamaño PSU habitual ≥ (CPU + GPU(s) + componentes seleccionados) × 1,30',
  ...
  ru: {
    ...
    variants: {
      ...
      psu: {
        summary: "Калькулятор PSU оценивает нагрузку по мощности CPU/GPU и прозрачным допускам для фактически выбранных компонентов.",
        inputs: "Мощность CPU и GPU, базовая нагрузка платы, RAM, накопители, вентиляторы, охлаждение, дополнительные устройства, вторая GPU, ручной запас и разгон.",
        calculation: "Допуски выбранных компонентов суммируются с мощностью CPU и GPU. Инструмент показывает запас 15 %, 30 % и 50 % и округляет результат до распространённой мощности PSU.",
        formula: "Планируемая мощность = распространённый размер PSU ≥ (CPU + GPU + выбранные компоненты) × 1,30",
